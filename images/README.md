...
## Testimonial profiles (drop-in replace)

Three demo profile illustrations are included. To replace them, copy your photos into this folder using these names:

- `profile-1.webp` / `.png` / `.jpg` / `.jpeg`
- `profile-2.webp` / `.png` / `.jpg` / `.jpeg`
- `profile-3.webp` / `.png` / `.jpg` / `.jpeg`

The website checks for those files automatically and replaces the matching demo profile. Square images work best; they are cropped into circles with `object-fit: cover`.
