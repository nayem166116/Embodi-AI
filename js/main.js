...
document.addEventListener('DOMContentLoaded', function () {
  ...
  if (slides.length && dotsWrap) {
    ...
    slides.forEach(function (_, i) {
      ...
      b.addEventListener('click', function () { stopAutoplay(); show(i); });
      dotsWrap.appendChild(b);
    });
    function show(i) {
      current = (i + slides.length) % slides.length;
      slides.forEach(function (s, idx) { s.classList.toggle('active', idx === current); });
      dotsWrap.querySelectorAll('button').forEach(function (d, idx) { d.classList.toggle('active', idx === current); });
    }
    var autoplay = !reduceMotion ? setInterval(function () { show(current + 1); }, 6000) : null;
    function stopAutoplay() { if (autoplay) { clearInterval(autoplay); autoplay = null; } }
    if (testiPrev) testiPrev.addEventListener('click', function () { stopAutoplay(); show(current - 1); });
    if (testiNext) testiNext.addEventListener('click', function () { stopAutoplay(); show(current + 1); });
  }
  ...
  if (heroVisual) {
    var heroSource = heroVideo ? heroVisual.querySelector('.hero-bg-video source[type="video/mp4"]') : null;
  ...
  /* Given a .mp4 data-src, also point the sibling <source type="video/webm"> at the matching
     .webm filename so WebM (smaller/faster) is preferred automatically when it exists; the
     browser silently falls back to the .mp4 source if no .webm is uploaded. */
  function webmUrlFor(mp4Url) {
    return mp4Url.replace(/\.mp4(\?.*)?$/i, '.webm$1');
  }
  ...
  function loadClip(thumb) {
    ...
    if (mp4Source && mp4Source.getAttribute('src') !== src) {
      mp4Source.setAttribute('src', src);
      if (webmSource) webmSource.setAttribute('src', webmUrlFor(src));
      video.load();
  ...
  if (primary && video) {
    var initialSource = video.querySelector('source[type="video/mp4"]') || video.querySelector('source');
